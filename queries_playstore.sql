...
SELECT app_name, rating, price FROM analytics WHERE min_installs <= 50 AND rating IS NOT NULL ORDER BY rating DESC;
...
SELECT app_name, min_installs/reviews AS proportion, min_installs, reviews FROM analytics WHERE min_installs >= 100000 ORDER BY proportion DESC LIMIT 1;
...
SELECT category, app_name, rating FROM analytics WHERE (rating, category) in (SELECT MAX(rating), category FROM analytics WHERE min_installs >= 50000 GROUP BY category);
...
SELECT app_name FROM analytics WHERE array_length(genres, 1) >= 2;
